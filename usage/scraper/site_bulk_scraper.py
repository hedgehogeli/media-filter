...
from site_utils.playwright_scraper import PlaywrightHandler
from site_utils.image_processing import ImageProcessor, MAX_SIZE_KB, HEADERS
...
class AlbumScraper:
    ...
    def process_album(self, album_url: str, shutdown_event: mp.Event):
        ...
        DESIRED_FILE_COUNT = 10  # this number includes previews
        existing_files = []
        if os.path.exists(self.subdir_path):
            existing_files = [
                f
                for f in os.listdir(self.subdir_path)
                if os.path.isfile(os.path.join(self.subdir_path, f))
                and not f.endswith(".log")
            ]
        existing_count = len(existing_files)
        if existing_count >= 1:
            self.logger.info(
                f"Directory already has {existing_count} files, marking as complete"
            )
            return AlbumResult(  # return a fake success
                album_url=self.album_url,
                album_id=self.album_id,
                num_previews=DESIRED_FILE_COUNT,
                num_files=DESIRED_FILE_COUNT,
                num_total=DESIRED_FILE_COUNT,
                incomplete=False,
            )
        else:
            try:
                album_html = PlaywrightHandler.fetch_page_content(
                    album_url + "?advanced=1"
                )
            except Exception as e:
                self.logger.error(f"Error fetching {album_url}: {e}")
                return AlbumResult(  # return a fake success
                    album_url=self.album_url,
                    album_id=self.album_id,
                    num_previews=DESIRED_FILE_COUNT,
            ...
            self.logger.info(
                f"Found {len(file_list)} files in HTML from {self.album_id}"
            )

            if len(file_list) > 0:
                num_files_to_grab = DESIRED_FILE_COUNT - existing_count
                file_list = random.sample(
                    file_list, min(num_files_to_grab, len(file_list))
                )
                self.logger.info(
                    f"Randomly selected {len(file_list)} files from available options"
                )
            else:
                self.logger.info(f"skipping album no files found")
                return AlbumResult(  # return a fake success
                    album_url=self.album_url,
                    album_id=self.album_id,
                    num_previews=DESIRED_FILE_COUNT,
        ...
        return AlbumResult(
            ...
            num_total=total_file_count,  # len(file_list)
...
def worker_function(
    ...
    )  # workers should ignore Ctrl + C SIGINT
...
def result_collector(
    ...
    while not shutdown_event.is_set() or not result_queue.empty():
        try:
            ...
            if result is None:
                logger.info("Result collector received sentinel, exiting")
                break
            ...
            if shutdown_event.is_set():
                # if shutdown requested and nothing new for a short while, exit loop
                logger.info(
                    "Shutdown event set and no results; result collector exiting"
                )
                break
    ...
    logger.info("Result collector terminated")
...
def main():
    ...
    n_workers = 9  # min(mp.cpu_count() - 1 or 1, 8)  # Cap at 8 workers
    ...
    try:
        ...
        logger.info(f"{len(pending_albums)} albums enqueued, waiting for completion...")
        # wait for the job_queue to be empty, i.e everything has been processed
        ...
        # when we send poison pills, we are waiting for the workers to:
        # fiinsh their last jobs and see the pills (when we run to completion)
        # see the interrupt flag, and gracefully exit (when we ctrl+c)
        logger.info(f"sending worker and collector poison pills")
        for _ in range(n_workers):
            job_queue.put(None)
        result_queue.put(None)  # collector sentinel
        ...
        for worker in workers:
            ...
            if worker.is_alive():
                ...
                worker.join(
                    timeout=5
                )  # this should be redundant, the above .join() should be blocking...
        ...
        logger.info("Final cleanup: ensuring all processes are terminated")

        # Signal collector thread to stop
        collector_thread.join(timeout=10)
        if collector_thread.is_alive():
            logger.warning("Collector thread did not exit in time")
        ...
        # Close multiprocessing queues and join their internal threads to avoid hang
        logger.info("Final cleanup: forcing queues to close")
        try:
            job_queue.close()
            job_queue.cancel_join_thread()
        except Exception:
            logger.exception("Error closing job_queue")
        try:
            result_queue.close()
            result_queue.cancel_join_thread()
        except Exception:
            logger.exception("Error closing result_queue")
...
if __name__ == "__main__":
    # Required for Windows, no longer necessary now we're on linux
    mp.freeze_support()
    main()
