# common utils for server and client
...
class ProcessedImage:
    ...
    filename: str
    processed_img: Optional[Any] = None
    error: Optional[str] = None
...
class ModelBatch:
    ...
    def get_tensor_batch(self) -> Tuple[Tuple[Dict, torch.Tensor], List[int]]:
        ...
        for idx, img in enumerate(self.images):
            if (
                img.error is None
                and img.processed_img is not None
            ):
                clip_tensors.append(img.processed_img[0])
                yolo_tensors.append(img.processed_img[1])
